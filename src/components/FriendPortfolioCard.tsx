...
export interface FriendPortfolioCardSummary {
  id: string;
  name: string;
  pctVsBenchmark: number;     // big headline figure, e.g. lifetime gain %
  benchmarkLabel?: string;    // subtitle for the big figure; defaults to "since first invested"
  positionsCount: number;
  lifetimeGainPct: number;    // overall portfolio gain since inception (replaces ytdPct)
}
  ...
  return (
    ...
    >
      ...
      <div className="border-t border-line mt-3.5 pt-3 text-[12px] text-fg-mid font-medium tabular-nums flex items-center justify-between">
        ...
        <span className={summary.lifetimeGainPct >= 0 ? "text-pos" : "text-neg"}>
          {sign}{summary.lifetimeGainPct.toFixed(1)}% all-time
        </span>
      </div>
    </Link>
  );
